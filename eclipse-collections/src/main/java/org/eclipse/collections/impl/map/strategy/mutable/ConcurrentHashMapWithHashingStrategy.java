...
public final class ConcurrentHashMapWithHashingStrategy<K, V>
        ...
        implements ConcurrentMutableMap<K, V>, Externalizable
{
    ...
    private static final AtomicReferenceFieldUpdater<ConcurrentHashMapWithHashingStrategy, AtomicReferenceArray> TABLE_UPDATER = AtomicReferenceFieldUpdater.newUpdater(ConcurrentHashMapWithHashingStrategy.class, AtomicReferenceArray.class, "table");
    private static final AtomicIntegerFieldUpdater<ConcurrentHashMapWithHashingStrategy> SIZE_UPDATER = AtomicIntegerFieldUpdater.newUpdater(ConcurrentHashMapWithHashingStrategy.class, "size");
    ...
    /**
     * @deprecated No argument default constructor used for serialization. Instantiating an ConcurrentHashMapWithHashingStrategy with
     * this constructor will have a null hashingStrategy and throw NullPointerException when used.
     */
    @Deprecated
    public ConcurrentHashMapWithHashingStrategy()
    {
    }

    public ConcurrentHashMapWithHashingStrategy(HashingStrategy<? super K> hashingStrategy)
    {
        this(hashingStrategy, DEFAULT_INITIAL_CAPACITY);
    }

    public ConcurrentHashMapWithHashingStrategy(HashingStrategy<? super K> hashingStrategy, int initialCapacity)
    {
    ...
    public static <K, V> ConcurrentHashMapWithHashingStrategy<K, V> newMap(HashingStrategy<? super K> hashingStrategy)
    {
        return new ConcurrentHashMapWithHashingStrategy<>(hashingStrategy);
    }

    public static <K, V> ConcurrentHashMapWithHashingStrategy<K, V> newMap(HashingStrategy<? super K> hashingStrategy, int newSize)
    {
        return new ConcurrentHashMapWithHashingStrategy<>(hashingStrategy, newSize);
    }
    ...
    public V putIfAbsent(K key, V value)
    {
        ...
        while (true)
        {
            ...
            int index = ConcurrentHashMapWithHashingStrategy.indexFor(hash, length);
    ...
    {
        int h = strategy.computeHashCode(key);
        h ^= h >>> 20 ^ h >>> 12;
        h ^= h >>> 7 ^ h >>> 4;
        return h;
    }
    ...
    public boolean remove(Object key, Object value)
    {
        ...
        while (true)
        {
            ...
            else
            {
                ...
                {
                    Object candidate = e.getKey();
                    if (strategy.equals(candidate, key) && e.getValue().equals(value))
                    {
    ...
    public V get(Object key)
    {
        int hash = this.hash(key);
        AtomicReferenceArray currentArray = this.table;
        int index = ConcurrentHashMapWithHashingStrategy.indexFor(hash, currentArray.length());
        Object o = currentArray.get(index);
        if (o == RESIZED || o == RESIZING)
        {
        ...
        {
            Object k;
            if ((k = e.key) == key || strategy.equals(key, k))
            {
                return e.value;
            }
        }
    ...
    private V slowGet(Object key, int hash, int index, AtomicReferenceArray currentArray)
    {
        ...
        {
            int length = currentArray.length();
            index = ConcurrentHashMapWithHashingStrategy.indexFor(hash, length);
            Object o = currentArray.get(index);
            if (o == RESIZED || o == RESIZING)
            {
                currentArray = this.helpWithResizeWhileCurrentIndex(currentArray, index);
    ...
    public void putAllInParallel(Map<? extends K, ? extends V> map, int chunks, Executor executor)
    {
        ...
        if (map instanceof ConcurrentHashMapWithHashingStrategy<?, ?> && chunks > 1 && map.size() > 50000)
        {
            ConcurrentHashMapWithHashingStrategy<K, V> incoming = (ConcurrentHashMapWithHashingStrategy<K, V>) map;
    ...
    public boolean replace(K key, V oldValue, V newValue)
    {
        ...
        {
            Object candidate = e.getKey();
            if (candidate == key || strategy.equals(candidate, key))
            {
    ...
    public int hashCode()
    {
        ...
        for (int i = 0; i < currentArray.length() - 1; i++)
        {
            ...
            while (e != null)
            {
                Object key = e.getKey();
                Object value = e.getValue();
                h += (key == null ? 0 : this.strategy.computeHashCode(key.hashCode())) ^ (value == null ? 0 : value.hashCode());
                e = e.getNext();
            }
        }
        return h;
    ...
    private abstract class HashIterator<E> implements Iterator<E>
    {
        ...
        {
            this.currentState = new IteratorState(ConcurrentHashMapWithHashingStrategy.this.table);
            this.findNext();
        }
        ...
        private void findNext()
        {
            while (this.index < this.currentState.end)
            {
                ...
                if (o == RESIZED || o == RESIZING)
                {
                    AtomicReferenceArray nextArray = ConcurrentHashMapWithHashingStrategy.this.helpWithResizeWhileCurrentIndex(this.currentState.currentTable, this.index);
        ...
        protected void removeByKey()
        {
            ...
            this.current = null;
            ConcurrentHashMapWithHashingStrategy.this.remove(key);
        }
        ...
        protected boolean removeByKeyValue()
        {
            ...
            this.current = null;
            return ConcurrentHashMapWithHashingStrategy.this.remove(key, val);
        }
    }
    ...
    private final class KeySet extends AbstractSet<K>
    {
        ...
        public int hashCode()
        {
            ...
            for (K key : this)
            {
                hashCode += ConcurrentHashMapWithHashingStrategy.this.strategy.computeHashCode(key);
            }
            return hashCode;
        }
        ...
        public int size()
        {
            return ConcurrentHashMapWithHashingStrategy.this.size();
        }

        @Override
        public boolean contains(Object o)
        {
            return ConcurrentHashMapWithHashingStrategy.this.containsKey(o);
        }

        @Override
        public boolean remove(Object o)
        {
            return ConcurrentHashMapWithHashingStrategy.this.remove(o) != null;
        }

        @Override
        public void clear()
        {
            ConcurrentHashMapWithHashingStrategy.this.clear();
        }
    }
    ...
    private final class Values extends AbstractCollection<V>
    {
        ...
        public boolean contains(Object o)
        {
            return ConcurrentHashMapWithHashingStrategy.this.containsValue(o);
        }
    ...
    private final class EntrySet extends AbstractSet<Map.Entry<K, V>>
    {
        ...
        public int hashCode()
        {
            return ConcurrentHashMapWithHashingStrategy.this.hashCode();
        }
        ...
        public boolean contains(Object o)
        {
            ...
            Entry<K, V> candidate = ConcurrentHashMapWithHashingStrategy.this.getEntry(e.getKey());
            return candidate != null && Objects.equals(e.getValue(), candidate.getValue());
        }
        ...
        public boolean remove(Object o)
        {
            ...
            Map.Entry<K, V> e = (Map.Entry<K, V>) o;
            return ConcurrentHashMapWithHashingStrategy.this.remove(e.getKey(), e.getValue());
        }
    ...
    private final class Entry<K, V> implements Map.Entry<K, V>
    {
        ...
        {
            throw new UnsupportedOperationException("ConcurrentHashMapWithHashingStrategy.Entry.setValue() not implemented. Use put() instead.");
        }
        ...
        public boolean equals(Object o)
        {
            ...
            K k1 = this.key;
            Object k2 = e.getKey();
            if (k1 == k2 || k1 != null && ConcurrentHashMapWithHashingStrategy.this.strategy.equals(k1, k2))
            {
                V v1 = this.value;
        ...
        public int hashCode()
        {
            return (this.key == null ? 0 : ConcurrentHashMapWithHashingStrategy.this.strategy.computeHashCode(this.key)) ^ (this.value == null ? 0 : this.value.hashCode());
        }
    ...
    public static <NK, NV> ConcurrentHashMapWithHashingStrategy<NK, NV> newMap(HashingStrategy<? super NK> hashingStrategy, Map<NK, NV> map)
    {
        ConcurrentHashMapWithHashingStrategy<NK, NV> result = new ConcurrentHashMapWithHashingStrategy<>(hashingStrategy, map.size());
        result.putAll(map);
        return result;
    }

    @Override
    public ConcurrentHashMapWithHashingStrategy<K, V> withKeyValue(K key, V value)
    {
        return (ConcurrentHashMapWithHashingStrategy<K, V>) super.withKeyValue(key, value);
    }

    @Override
    public ConcurrentHashMapWithHashingStrategy<K, V> withMap(Map<? extends K, ? extends V> map)
    {
        return (ConcurrentHashMapWithHashingStrategy<K, V>) super.withMap(map);
    }

    @Override
    public ConcurrentHashMapWithHashingStrategy<K, V> withMapIterable(MapIterable<? extends K, ? extends V> mapIterable)
    {
        return (ConcurrentHashMapWithHashingStrategy<K, V>) super.withMapIterable(mapIterable);
    }

    @Override
    public ConcurrentHashMapWithHashingStrategy<K, V> withAllKeyValues(Iterable<? extends Pair<? extends K, ? extends V>> keyValues)
    {
        return (ConcurrentHashMapWithHashingStrategy<K, V>) super.withAllKeyValues(keyValues);
    }

    @Override
    public ConcurrentHashMapWithHashingStrategy<K, V> withAllKeyValueArguments(Pair<? extends K, ? extends V>... keyValues)
    {
        return (ConcurrentHashMapWithHashingStrategy<K, V>) super.withAllKeyValueArguments(keyValues);
    }

    @Override
    public ConcurrentHashMapWithHashingStrategy<K, V> withoutKey(K key)
    {
        return (ConcurrentHashMapWithHashingStrategy<K, V>) super.withoutKey(key);
    }

    @Override
    public ConcurrentHashMapWithHashingStrategy<K, V> withoutAllKeys(Iterable<? extends K> keys)
    {
        return (ConcurrentHashMapWithHashingStrategy<K, V>) super.withoutAllKeys(keys);
    }

    @Override
    public MutableMap<K, V> clone()
    {
        return ConcurrentHashMapWithHashingStrategy.newMap(this.strategy, this);
    }

    @Override
    public <K, V> MutableMap<K, V> newEmpty(int capacity)
    {
        return new ConcurrentHashMapWithHashingStrategy<>(strategy, capacity);
    }
    ...
    {
        return new ConcurrentHashMapWithHashingStrategy<>(strategy);
    }
